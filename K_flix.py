...
if st.sidebar.checkbox("Recommandation de films"): 
        X = df_victory1.drop(['title','movieId','year'], axis=1)
        y = df_victory1['title']
        ...
        #name = st.text_input("")
        f = df_victory1[df_victory1["title"]== name].index[0]
        film = df_victory1[df_victory1.index == f]
        voisin1 = modelKNN.kneighbors(film.loc[:,['year', 'Autres', 'Action', 'Adventure', 'Animation', 'Children', 'Comedy', 'Crime', 'Documentary',
                                          'Drama', 'Fantasy', 'Film-Noir', 'Horror', 'IMAX', 'Musical', 'Mystery','Romance',
                                          'Sci-Fi', 'Thriller', 'War', 'Western', 'rating']]) [1][0]
        for i in voisin1:
           neigh = pd.DataFrame(df_victory1[df_victory1.index == i]['title'])
